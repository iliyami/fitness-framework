...
public class ExercisePlanCLI {
	...
	private static void UpdateExercisePlanParser(final Scanner sc) {
		try {
			...
			if (type.equals(ExerciseType.CARDIOVASCULAR)) {
				...
				final ExercisePlanInventory exercisePlanInventory = ExercisePlanInventory.getInstance();
				CardioExercisePlan exercisePlan = (CardioExercisePlan) exercisePlanInventory
						.getExercisePlanById(Integer.parseInt(args[0]));
			...
			if (type.equals(ExerciseType.MUSCULAR)) {
				...
				final ExercisePlanInventory exercisePlanInventory = ExercisePlanInventory.getInstance();
				MuscularExercisePlan exercisePlan = (MuscularExercisePlan) exercisePlanInventory
						.getExercisePlanById(Integer.parseInt(args[0]));
	...
	private static void DeleteExercisePlanParser(final Scanner sc) {
		try {
			...
			final ExercisePlanInventory exercisePlanInventory = ExercisePlanInventory.getInstance();
			final ExercisePlan exercisePlan = exercisePlanInventory.getExercisePlanById(Integer.parseInt(args[0]));
			if (exercisePlan == null) {
				System.out.println("  Exercise Plan Doesn't Exist!\n");

			} else {
				exercisePlanInventory.deleteExercisePlan(exercisePlan);
